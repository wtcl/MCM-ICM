...
while X[-1]<43300:
    ...
    num+=1
    vvv=V[-1]
    while WW[-1]>=0 and X[-1]<43300 and Pmax_cp==1:
    ...
    while V[-1] >= ((max(V) - vvv) / 2.73 + vvv) and X[-1]<43300:
    ...
    while WW[-1] < 24000*(0.95)**(num) and X[-1]<43300:
...
data.to_csv(".//UCI.csv",index=False)
timee=time.time()
print(timee-times)
