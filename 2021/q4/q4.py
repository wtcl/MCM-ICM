...
print(data_full)
data_full.to_csv("full0.csv",index=False)
...
inferlist=[]
for i in range(5):
    inferlist.append(data_infer[data_infer['year'] == (1963 + i)])
data_infer=pd.concat(inferlist,ignore_index=False)
...
for ii in range(len(followers)):
    ...
    list_data=[]
    for j in range(5):
        list_data.append(data_fulli[data_fulli['year']==(1963+j)])
    newdata=pd.concat(list_data,ignore_index=False)
    tdata=[]
...
exdata.to_csv("66915.csv",index=False)
...
for i in range(len(followers)):
    ...
    list_data0=[]
    for j in range(5):
        list_data0.append(data_fulli[data_fulli['year']==(1963+j)])
        print(data_fulli[data_fulli['year']==(1963+j)])
    newdata0=pd.concat(list_data0,ignore_index=False)
    list_data1 = []
    for j in range(5):
        list_data1.append(data_fulli[data_fulli['year'] == (1963 - j)])
    newdata1 = pd.concat(list_data1, ignore_index=False)
    tdata=[]
...
print(similarity_data)
exdata=pd.DataFrame({"id":followers,"similarity_data":similarity_data})
exdata.to_csv("66915_followerself.csv",index=False)